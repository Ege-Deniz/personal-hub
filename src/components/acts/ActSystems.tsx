...
// ACT 5 — SELECTED SYSTEMS. Typographic catalog: index numeral · name ·
// one-line · stack tags · status. Honest data only — no fabricated metrics.
// Catalog data ported from SelectedSystems.tsx; the Neural Engine entry is
// updated to describe the GPGPU field this rebuild ships.
...
const SYSTEMS: System[] = [
  ...
  },
  {
    index: "04",
    name: "Neural Engine",
    line: "The field behind this page: a GPGPU particle brain — tens of thousands of simulated points with curl-noise drift, cursor physics, and a custom post chain.",
...
function SystemRow({ system, i }: { system: System; i: number }) {
  const Wrapper = (system.href ? "a" : "div") as "a";
  ...
  return (
    ...
    >
      <Wrapper
        {...wrapperProps}
        className="group relative block border-t border-white/[0.08] py-8 transition-colors duration-500 hover:border-cyan/30 sm:py-10"
      >
        ...
        <div className="flex items-baseline gap-6 sm:gap-12">
          <span
            aria-hidden="true"
            className="text-outline select-none font-display text-[clamp(2.4rem,5vw,4.2rem)] font-extrabold leading-none tracking-[-0.04em] transition-colors duration-500"
          >
            {system.index}
          </span>
...
export default function ActSystems() {
  ...
  return (
    ...
    >
      ...
      {/* Credibility ledger — facts only */}
      <div className="tnum mt-6 flex flex-wrap gap-x-8 gap-y-2 font-mono text-[0.6rem] uppercase tracking-[0.25em] text-white/25">
        <span>05 systems indexed</span>
        <span>03 live</span>
        <span>Thesis defense 06/2026</span>
      </div>
    </section>
  );
}
